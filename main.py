...
class SaTEP:
    class Function:
        ...
        def SavePath(self, driverManager) -> ndarray:
            ...
            isExist    = exists(driverPath)
            if not isExist:
                path = driverManager.install()
                copy(path, driverPath)
                return array([path, isExist])
            return array([driverPath, isExist])
    ...
    class GUI(Function):
        def __init__(self) -> None:
            ...
            self.bg     = "#dedede"
            self.driver = None
            super().__init__(self.root, self.bg)

            path, isExist = self.Web()
            self.SaTEP()
        ...
        def Web(self):
            for browser in browsers():
                ...
                if type == "chrome":
                    ...
                    path, isExist = self.SavePath(ChromeDriverManager())
                    self.driver = Chrome(service=ChromeService(path), options=options)
                    break
                if type == "msedge":
                    options = EdgeOptions()
                    options.add_experimental_option("detach", True)
                    path, isExist = self.SavePath(EdgeChromiumDriverManager())
